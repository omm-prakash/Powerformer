...
def extractData(data_path:str, 
    ...
    pattern = re.compile(r'.*/case-(\d+)-\(t(\d+)_l(\d+)\)\.csv')
    ...
    for file in data_files: 
        ...
        if match:
            case_number, fault_type, fault_location = match.groups()          
            case_number = int(case_number)
            fault_location = int(fault_location)
            fault_type = int(fault_type)  
        else:
            continue  # This avoids crashing
        if fault_location in ignored_fault_locations or \
            fault_type in ignored_fault_types or \
            ((case_range is not None) and (not case_range[0]<=case_number<=case_range[1])):
            continue
